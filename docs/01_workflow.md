...
Las 12 instancias estáticas definidas en el archivo fuente se compilan en formato OpenType con curvas PostScript (OTF/CFF) y WebOpenFontFormat (WOFF) y se ubican en:
* **Directorio de Prueba/Individual:** `builds/otf/proof/` y `builds/woff/proof/`
...
1. Empaqueta los archivos OTF y WOFF finales en archivos comprimidos `.zip` (`LEMPro_OTF.zip` y `LEMPro_WOFF.zip`), incluyendo el archivo `EULA.txt`.
2. Guarda los paquetes en: `builds/release/`.
